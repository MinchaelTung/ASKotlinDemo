...
//    类型：属性的数据类型
//    表达式：委托代理类
//    by 关键字之后的表达式就是委托, 属性的 get() 方法(以及set() 方法)将被委托给这个对象的
//    getValue() 和 setValue() 方法。属性委托不必实现任何接口, 但必须提供 getValue() 函数
//    (对于 var属性,还需要 setValue() 函数)。

//    定义一个被委托的类
//    该类需要包含 getValue() 方法和 setValue() 方法，且参数 thisRef 为进行委托的类的对象，
//    prop 为进行委托的属性的对象。
   ...
   // import kotlin.reflect.KProperty
    ...
    fun main1223(args: Array<String>) {
        val e = Example()
...
//    import kotlin.properties.Delegates
    ...
    fun main123ww(args: Array<String>) {
        val user = User()
        user.name = "第一次赋值"
        user.name = "第二次赋值"
...
//    对于一个值可变(mutable)属性(也就是说,var 属性),除 getValue()函数之外,它的委托还必须
// 另外再提供一个名为setValue()的函数, 这个函数接受以下参数:
//    property —— 必须是类型 KProperty<*> 或其超类型
...
// 第一个参数 this 引用到外部类 C 的实例而 this::prop 是 KProperty 类型的反射对象，
// 该对象描述 prop 自身。
...
// 那么会调用该函数来 创建属性委托实例。
//    provideDelegate 的一个可能的使用场景是在创建属性时
// （而不仅在其 getter 或 setter 中）检查属性一致性。
